...
@WebServlet(name = "corona", urlPatterns = {"/coronaurl", "/testurl"}, loadOnStartup = 1, 
						   initParams = {
								   					@WebInitParam(name = "p1" , value = "val1"),
								   					@WebInitParam(name = "p2", value = "val2")
						   							}
							)
//@WebServlet(urlPatterns = "/coronaurl")
//@WebServlet(urlPatterns = "/coronaurl")
public class CoronaRegisterServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(count!=0) {
				...
				pw.println("<h1 style='color:green'>init param value using Annotaion</h1>"+config.getInitParameter("p1"));
				pw.println("<h1 style='color:green'>init param value using Annotaion</h1>"+config.getInitParameter("p2"));
	...
	private Connection getPooledConnection() throws Exception {
		...
		ds = (DataSource) context.lookup("java:/comp/env/DsJndi");	// Tomcat
